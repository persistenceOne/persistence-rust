...
pub struct Channel {
    ...
    #[serde(
        serialize_with = "state_serde::serialize",
        deserialize_with = "state_serde::deserialize"
    )]
    pub state: i32,
    /// whether the channel is ordered or unordered
    #[prost(enumeration = "Order", tag = "2")]
    #[serde(
        serialize_with = "order_serde::serialize",
        deserialize_with = "order_serde::deserialize"
    )]
    pub ordering: i32,
...
pub mod state_serde {
        ...
        D: Deserializer<'de>,
    {
        ...
        let enum_value = State::from_str_name(&s).unwrap();
    ...
    where
        S: Serializer,
    {
        let s: State = State::from_i32(*value).unwrap();
...
pub mod order_serde {
    ...
    {
        let s = String::deserialize(deserializer)?;
        let enum_value = Order::from_str_name(&s).unwrap();
        let int_value: T = enum_value.into();
        return Ok(int_value);
    }
    pub fn serialize<S>(value: &i32, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s: Order = Order::from_i32(*value).unwrap();
        serializer.serialize_str(s.as_str_name())
    }
}
impl Order {
...
pub struct MsgRecvPacketResponse {
    ...
    #[serde(
        serialize_with = "response_result_type_serde::serialize",
        deserialize_with = "response_result_type_serde::deserialize"
    )]
    pub result: i32,
}
...
pub mod response_result_type_serde {
    use super::ResponseResultType;
    use serde::{Deserialize, Deserializer, Serializer};
    pub fn deserialize<'de, T, D>(deserializer: D) -> std::result::Result<T, D::Error>
    where
        T: From<ResponseResultType>,
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        let enum_value = ResponseResultType::from_str_name(&s).unwrap();
        let int_value: T = enum_value.into();
        return Ok(int_value);
    }
    pub fn serialize<S>(value: &i32, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s: ResponseResultType = ResponseResultType::from_i32(*value).unwrap();
        serializer.serialize_str(s.as_str_name())
    }
}
