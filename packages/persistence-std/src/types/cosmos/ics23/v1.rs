...
pub struct LeafOp {
    ...
    #[serde(
        serialize_with = "hash_op_serde::serialize",
        deserialize_with = "hash_op_serde::deserialize"
    )]
    pub hash: i32,
    ...
    #[serde(
        serialize_with = "length_op_serde::serialize",
        deserialize_with = "length_op_serde::deserialize"
    )]
    pub length: i32,
...
pub mod hash_op_serde {
        ...
        D: Deserializer<'de>,
    {
        ...
        let enum_value = HashOp::from_str_name(&s).unwrap();
    ...
    where
        S: Serializer,
    {
        let s: HashOp = HashOp::from_i32(*value).unwrap();
...
pub mod length_op_serde {
    ...
    {
        let s = String::deserialize(deserializer)?;
        let enum_value = LengthOp::from_str_name(&s).unwrap();
        let int_value: T = enum_value.into();
        return Ok(int_value);
    }
    pub fn serialize<S>(value: &i32, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s: LengthOp = LengthOp::from_i32(*value).unwrap();
        serializer.serialize_str(s.as_str_name())
    }
}
impl LengthOp {
