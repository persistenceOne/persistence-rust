...
pub struct Proposal {
    ...
    #[serde(
        serialize_with = "proposal_status_serde::serialize",
        deserialize_with = "proposal_status_serde::deserialize"
    )]
    pub status: i32,
    ...
    #[serde(
        serialize_with = "proposal_executor_result_serde::serialize",
        deserialize_with = "proposal_executor_result_serde::deserialize"
    )]
...
pub struct Vote {
    ...
    #[serde(
        serialize_with = "vote_option_serde::serialize",
        deserialize_with = "vote_option_serde::deserialize"
    )]
    pub option: i32,
...
pub mod vote_option_serde {
        ...
        D: Deserializer<'de>,
    {
        ...
        let enum_value = VoteOption::from_str_name(&s).unwrap();
    ...
    where
        S: Serializer,
    {
        let s: VoteOption = VoteOption::from_i32(*value).unwrap();
...
pub mod proposal_executor_result_serde {
    use super::ProposalExecutorResult;
    use serde::{Deserialize, Deserializer, Serializer};
    pub fn deserialize<'de, T, D>(deserializer: D) -> std::result::Result<T, D::Error>
    where
        T: From<ProposalExecutorResult>,
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        let enum_value = ProposalExecutorResult::from_str_name(&s).unwrap();
        let int_value: T = enum_value.into();
        return Ok(int_value);
    }
    pub fn serialize<S>(value: &i32, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s: ProposalExecutorResult = ProposalExecutorResult::from_i32(*value).unwrap();
        serializer.serialize_str(s.as_str_name())
    }
}
...
pub struct MsgSubmitProposal {
    ...
    #[serde(
        serialize_with = "exec_serde::serialize",
        deserialize_with = "exec_serde::deserialize"
    )]
    pub exec: i32,
    /// title is the title of the proposal.
    ///
...
pub mod exec_serde {
    ...
    {
        let s = String::deserialize(deserializer)?;
        let enum_value = Exec::from_str_name(&s).unwrap();
        let int_value: T = enum_value.into();
        return Ok(int_value);
    }
    pub fn serialize<S>(value: &i32, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s: Exec = Exec::from_i32(*value).unwrap();
        serializer.serialize_str(s.as_str_name())
    }
}
impl Exec {
