...
pub struct Vote {
    ...
    #[serde(
        serialize_with = "signed_msg_type_serde::serialize",
        deserialize_with = "signed_msg_type_serde::deserialize"
    )]
    pub r#type: i32,
    #[prost(int64, tag = "2")]
    #[serde(
...
pub struct CommitSig {
    ...
    #[serde(
        serialize_with = "block_id_flag_serde::serialize",
        deserialize_with = "block_id_flag_serde::deserialize"
    )]
...
pub mod block_id_flag_serde {
        ...
        D: Deserializer<'de>,
    {
        ...
        let enum_value = BlockIdFlag::from_str_name(&s).unwrap();
    ...
    where
        S: Serializer,
    {
        let s: BlockIdFlag = BlockIdFlag::from_i32(*value).unwrap();
...
pub mod signed_msg_type_serde {
    use super::SignedMsgType;
    use serde::{Deserialize, Deserializer, Serializer};
    pub fn deserialize<'de, T, D>(deserializer: D) -> std::result::Result<T, D::Error>
    where
        T: From<SignedMsgType>,
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        let enum_value = SignedMsgType::from_str_name(&s).unwrap();
        let int_value: T = enum_value.into();
        return Ok(int_value);
    }
    pub fn serialize<S>(value: &i32, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s: SignedMsgType = SignedMsgType::from_i32(*value).unwrap();
        serializer.serialize_str(s.as_str_name())
    }
}
impl SignedMsgType {
